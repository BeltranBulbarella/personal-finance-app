      ...
      try {
        ...
        if (!data) {
          if (
            ...
            this.logger.warn(
              `API key ${getCurrentApiKey()} rate limit reached, switching to next key.`,
            );
            switchToNextApiKey();
          }
        ...
        this.logger.error(
          `Failed to fetch data for ${stock.symbol} with API key ${getCurrentApiKey()}: ${error.message}`,
        );
      }
    }
  ...
  private async fetchAndStoreCryptoPrice(crypto: {
...

    {
      ...
      try {
        ...
        this.logger.error(
          `Failed to fetch data for ${crypto.symbol} with API key ${getCurrentApiKey()}: ${error.message}`,
        );
      }
    }
